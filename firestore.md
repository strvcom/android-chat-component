...
There are three top level collections in Firestore:
* Conversations
* Messages
* Users
...
* `last_message` - container for the minimum necessary info about the last message
    ...
    * `timestamp` - a timestamp of the message defined as a long number (not a firestore date timestamp as this slows down the queries considerably)
* `members` [] - an array of users who are members of this conversation
* `seen` [] - an array of users who read the last message in this conversation

#### Messages

The `messages` is the top level collection in firestore
`messages`
...
* `data`
    * `message` - in case of a text message, the text of the message
    * `image_url` - in case of an image message, the url of the image
